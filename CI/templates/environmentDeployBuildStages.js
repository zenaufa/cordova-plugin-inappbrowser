	...
	if(pipelineStatus=="StartedBuilding" || pipelineStatus=="Success") {
		checkResponse.BuildingResponse.ApplicationPlatform.forEach( (aux) => {
			aux.PlatformProgress.forEach( (element) => {
				// var auxBar = null
				// if (bars[aux.Name] == undefined) {
				// 	bars[aux.Name] = {}	
				// } 
				// if (bars[aux.Name][element.Name] == undefined) {
				// 	console.log("Creating bar");
				// 	auxBar = multibar.create(100, 0);
				// 	bars[aux.Name][element.Name] = auxBar;
				// 	console.log(auxBar);
				// } 
				// else {
				// 	auxBar = bars[aux.Name][element.Name];
				// 	console.log(auxBar);
				// }
				// console.log(auxBar);
				// auxBar.update(element.progress, {filename: element.Name + " " + aux.Name });
				console.log("App " + element.Name + " " + aux.Name + " progress " + element.Progress);
			});
		});
	}
